...
def main():    
    vec = tf.compat.v1.placeholder("float", [config.batch_size, config.max_phr_len, config.input_features])
    ...
    # seqlen = tf.compat.v1.placeholder("float", [config.batch_size, 256])
    # with tf.compat.v1.variable_scope('singer_Model') as scope:
    #     singer_emb, outs_sing = singer_network(vec, is_train)
    # with tf.compat.v1.variable_scope('f0_Model') as scope:
    #     outs_f0 = f0_network(vec, is_train)
    # with tf.compat.v1.variable_scope('phone_Model') as scope:
    #     outs_pho = phone_network(vec, is_train)
    with tf.compat.v1.variable_scope('full_Model') as scope:
