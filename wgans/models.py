...
class WGANSing(Model):
    ...
    def get_placeholders(self):
        ...
        self.output_placeholder = tf.compat.v1.placeholder(tf.float32, shape=(config.batch_size, config.max_phr_len, config.output_features),
                                           name='output_placeholder')       


        self.phoneme_labels = tf.compat.v1.placeholder(tf.int32, shape=(config.batch_size, config.max_phr_len),
        ...
        self.f0_placeholder = tf.compat.v1.placeholder(tf.float32, shape=(config.batch_size, config.max_phr_len, 1),
                                        name='f0_placeholder')


        self.singer_labels = tf.compat.v1.placeholder(tf.float32, shape=(config.batch_size),name='singer_placeholder')
...
#             pho_target = pho_target[:1111]
    ...
    def test_file_hdf5(self, file_name, singer_index, ground=False, plot=False):
        """
        Function to extract multi pitch from file. Currently supports only HDF5 files.
        """
        sess = tf.Session()
    ...
    def model(self):
        ...
        with tf.compat.v1.variable_scope('Final_Model') as scope:
